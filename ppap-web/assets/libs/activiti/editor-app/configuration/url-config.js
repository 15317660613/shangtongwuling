...
KISBPM.URL = {
    ...
    getUser : function () {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/stwl-user';
    },
    //获取角色列表，应用范围：任务受理人编辑和受理组编辑
    getRole : function () {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/stwl-role/list';
    },
    //根据用户ID获取用户信息，应用范围：任务受理人展示
    getUserById : function (userId) {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/stwl-user/' + userId;
    },
    //根据角色ID获取角色信息，应用范围：受理组展示
    getRoleById : function (rId) {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/stwl-role/' + rId;
    },
    //获取oldForm关联表单列表，应用范围：关联表单编辑
    getForm : function () {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/form?pageNo=1&pageSize=500&design=0';
    },
    //获取newForm关联表单列表，应用范围：关联表单编辑
    getMobileForm : function () {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/form?pageNo=1&pageSize=500&design=1';
    },
    ...
    getOrgList : function () {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/stwl-org/list';
    },
    //根据组织机构ID获得该机构下所有用户列表，应用范围：任务受理人编辑
    getUserByOrgId : function () {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/stwl-user/list-with-org';
    },
    //根据角色ID获得该角色下所有用户列表，应用范围：任务受理人编辑
    getUserByRoleId : function (rId) {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/stwl-user/page/'+rId+'.do';
    },
    //获取跳转条件列表
    getCondition : function () {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/process/condition';
    },
    //根据code获取跳转条件名称
    getConditionByCode : function (code) {
        return ACTIVITI.CONFIG.contextRoot + '/api-edu/process/condition/' + code;
    }
