layui.use(['index', 'laydate', 'form', 'table', 'formSelects', 'upload'], function() {
    ...
    function setDistributionBar(data) {
        admin.req("/api-edu/risk/analysis/supplier-distribution?functionCodeList=" + data.ywmk + "&supplierCodeList=&levelList=" + data.fxcd + "&riskYear=" + data.date + "&scope=" + data.sex, {}, function(res) {
    ...
    function setDistributionPie(data) {
        admin.req("/api-edu/risk/analysis/supplier-pie?functionCodeList=" + data.ywmk + "&supplierCodeList=&levelList=" + data.fxcd + "&riskYear=" + data.date + "&scope=" + data.sex, {}, function(res) {
            // 判断业务模块是否为空
            if (!data.ywmk) {
                var btn = "";
        ...
        var optionBar = {
            ...
            tooltip: {
                ...
                formatter: function(params, ticket, callback) {
                    var html =
                        '<div style="position: relative;"><h1 style="font-size:20px;font-family:Bebas;color:rgba(119,119,119,1);line-height:26px;text-align: center;">' +
                        params[0].value +
                        '</h1><h5 style="font-size:14px;font-family:PingFangSC-Regular;font-weight:400;color:rgba(0,0,0,0.65);line-height:22px;">2018年' +
                        params[0].axisValue +
                        '总数据</h5><div style="position: absolute;bottom:-10px;left: 50%;transform: translateX(-10px);width:0;height:0;border-top: 14px solid #fff;border-left:10px solid transparent;border-right:10px solid transparent;"></div></div>';
                    return html;
                },
    ...
    var renderTable = function(num, data, content) {
        ...
        table.render({
            elem: '#' + content,
            url: admin.formatUrl('/api-edu/risk/analysis/supplier-detail?type=' + num + "&functionCodeList=" + data.ywmk + "&supplierCodeList=" + data.gysdm + "&levelList=" + data.fxcd + "&riskYear=" + data.date + "&scope=" + data.sex),
    ...
    var renderTable1 = function() {
        table.render({
            ...
            elem: '#tableContent1'
                ...
                ,
            // data: res.data,
            url: admin.formatUrl('/api-edu/risk/analysis/email/history')
                // ,toolbar: '#toolbarDemo'
    ...
    // 风险信息推送
    $("#push").click(function() {
        admin.req("/api-edu/risk/analysis/email?functionCodeList=" + saveArr.ywmk + "&supplierCodeList=" + saveArr.gysdm + "&levelList=" + saveArr.fxcd + "&riskYear=" + saveArr.date + "&scope=" + saveArr.sex, {}, function(res) {
            if (res.ok) {
                layer.msg(res.message);
            } else {
                layer.msg(res.message);
    ...
    $("#exportSingle").click(function() {
        window.location.href = "/api-edu/risk/analysis/excel/single?type=" + type + "&functionCodeList=" + saveArr.ywmk + "&supplierCodeList=" + saveArr.gysdm + "&levelList=" + saveArr.fxcd + "&riskYear=" + saveArr.date + "&scope=" + saveArr.sex
    });
    // 总清单导出
    $("#exportList").click(function() {
        window.location.href = "/api-edu/risk/analysis/excel/all?type=0&functionCodeList=" + saveArr.ywmk + "&supplierCodeList=" + saveArr.gysdm + "&levelList=" + saveArr.fxcd + "&riskYear=" + saveArr.date + "&scope=" + saveArr.sex
            // admin.req("/api-edu/risk/analysis/excel/all?type=0&functionCodeList=" + saveArr.ywmk + "&supplierCodeList=" + saveArr.gysdm + "&levelList=" + saveArr.fxcd + "&riskYear=" + saveArr.date + "&scope=" + saveArr.sex, {}, function(res) {
    ...
    $("#pieBody").on('click', 'button', function() {
        ...
        if (!saveArr.fxcd) {
            admin.req("/api-edu/risk/analysis/supplier-pie?type=0&functionCodeList=" + saveArr.ywmk + "&supplierCodeList=&levelList=" + $(this).val() + "&riskYear=" + barDate + "&scope=" + saveArr.sex, {}, function(res) {
                if (res.ok) {
                    var json = eval(res.data);
                    setValues_2(json);
                }
