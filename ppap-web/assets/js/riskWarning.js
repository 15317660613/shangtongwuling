layui.use(['index', 'laydate', 'form', 'table', 'formSelects', 'upload'], function() {
    ...
    var renderTable3 = function(data) {
        ...
        table.render({
            ...
            elem: '#demo-table2',
            where: data,
            url: admin.formatUrl("/api-edu/risk/warning/detail/list"),
    ...
    // 设置影响机型下拉框数据
    admin.req("/api-edu/risk/warning/affectcarType", {}, function(res) {
        if (res.ok) {
            var option = "<option value=''></option>";
    ...
    // 设置主要影响产品下拉框数据
    admin.req("/api-edu/risk/warning/affectparts", {}, function(res) {
        if (res.ok) {
            var option = "<option value=''></option>";
            for (var i = 0; i < res.data.length; i++) {
                option += "<option value='" + res.data[i] + "'>" + res.data[i] + "</option>"
    ...
    // 设置SQE下拉框数据
    admin.req("/api-edu/risk/warning/sqname", {}, function(res) {
        if (res.ok) {
            var option = "<option value=''></option>";
            for (var i = 0; i < res.data.length; i++) {
                option += "<option value='" + res.data[i] + "'>" + res.data[i] + "</option>"
    ...
    // 设置风险预警状态下拉框数据
    admin.req("/api-edu/risk/warning/state", {}, function(res) {
        if (res.ok) {
            var option = "<option value=''></option>";
            for (var i = 0; i < res.data.length; i++) {
    ...
    table.on('tool(demo-table2)', function(obj) {
        ...
        if (layEvent === 'download') {
            if (data.fileId) {
                ...
                window.location.href = admin.formatUrl("/api-edu/risk/warning/report/download?fileId=" + data.fileId);
                admin.removeLoading("body");
            } else {
        ...
        if (layEvent === 'view') {
            if (data.fileId) {
                if (data.fileType == "pic" || data.fileType == "pdf") {
                    admin.showLoading("body");
                    var url = admin.formatUrl("/api-edu/risk/warning/report/preview?fileId=" + data.fileId);
                    name = "";
    ...
    $("#wanningExport").click(function() {
        ...
        setTimeout(function() {
            ...
            formData.append("functionCode", saveArr3.functionCode);
            ...
            xhr.open('POST', admin.formatUrl("/api-edu/risk/warning/excel"), true);
    ...
    function setFormValue(data) {
        ...
        // 设置sq负责人
        admin.req("/api-edu/risk/warning/sqname-fuzzy?username=", {}, function(res) {
    ...
    form.on('submit(warningSave)', function(data) {
        ...
        // 发送请求
        admin.req('/api-edu/risk/warning', d, function(data) {
    ...
    function setRiskWarningBar(data) {
        admin.req("/api-edu/risk/warning/detial/bar?riskTime=" + data.riskTime + "&supplierCode=" + data.supplierCode + "&functionCodeList=" + data.functionCodeList + "&riskLevelCode=" + data.riskLevelCode + "&affectcarType=" + data.affectcarType + "&affectParts=" + data.affectParts + "&sqName=" + data.sqName + "&state=" + data.state, {}, function(res) {
    ...
    function setRiskWarningPie(data) {
        admin.req("/api-edu/risk/warning/detial/pie?riskTime=" + data.riskTime + "&supplierCode=" + data.supplierCode + "&functionCodeList=" + data.functionCodeList + "&riskLevelCode=" + data.riskLevelCode + "&affectcarType=" + data.affectcarType + "&affectParts=" + data.affectParts + "&sqName=" + data.sqName + "&state=" + data.state, {}, function(res) {
            var json = eval(res.data);
            setValues_5(json);
        });
    }
