layui.use(['index', 'laydate', 'form', 'table', 'formSelects', 'upload'], function() {
    ...
    // 得到上个月-即每月13日运算后台算法程序，并更新显示上个月的数据，在此之前均应显示上上个月的数据；
    function getPreMonth13(date, type) {
        var year = date.getFullYear(); //获取当前日期的年份
        var month = date.getMonth() + 1; //获取当前日期的月份
        var days = new Date(year, month, 0);
    ...
    // 得到上上个月
    function getPreMonth2(date, type) {
        var year = date.getFullYear(); //获取当前日期的年份
        var month = date.getMonth() + 1; //获取当前日期的月份
        var days = new Date(year, month, 0);
        var dayNows = date.getDate(); //获取当前日期中月的天数
    ...
    function getBtn() {
        ...
        if (curUser.roleIds.indexOf("1") > -1) {
            btn += "<button class=\"layui-btn layui-btn-sm layui-btn-primary\" style=\"padding: 0 30px;\" id=\"supplierManage\">风险供应商管理</button>\n" +
                "                                    <button class=\"layui-btn layui-btn-sm layui-btn-primary\" id=\"exportInsideFile\" style=\"padding: 0 30px;\">风险供应商内部数据</button>\n" +
                "                                    <button class=\"layui-btn layui-btn-sm layui-btn-primary\" id=\"executeInsideTask\" style=\"padding: 0 30px;\">执行内部风险预警</button>"
        }
        $("#monitorBtn").append(btn);
    }
    ...
    //     admin.req("api/risk/analysis/supplier/" + val.value, {}, function(res) {
    //         data = res.data;
    //         // $("#gysmc2").val(data.suppliername);
    //         if (isAdd) {
    ...
    laydate.render({
        type: 'month',
        elem: '#date-2',
        value: getPreMonth13(new Date(), 0),
    ...
    $("#reset1").click(function() {
        ...
        $("#date-2").val(getPreMonth13(new Date(), 0));
    ...
    $("#reset").click(function() {
        ...
        $("#dateDetail").val(getPreMonth13(new Date(), 0));
        $("#searchAll1").trigger("click");
    });
    // 全局查询结果变量
    var response;
    ...
    var renderSupplierManageTable = function(tableR, data) {
        ...
        table.render({
            ...
            id: 'supplierTable',
            where: data,
            url: admin.formatUrl('api/risk/monitior/risk-supplier/page'),
    ...
    function setSupplierStatus(data) {
        ...
        admin.req("api/risk/monitior/supplier-status?functionNameList=" + data.ywmk2 + "&supplierCode=" + data.gysdm2 + "&riskYear=" + data.date, {}, function(res) {
            if (res.ok) {
                response = res.data;
    ...
    function getSupplierManageCols(riskYearLast, riskYearNow) {
        var tableR = [
            [{
                ...
                templet: function(d) {
                    if (d.isMonitorNow == 1) {
                        return '<input id="' + d.riskYearNow + '" partsName="' + d.partsName + '" functionCode="' + d.functionCode + '" supplierName="' + d.supplierName + '" value="' + d.supplierCode + '" type="checkbox" name="' + d + '"  lay-skin="switch" lay-filter="bjDemo" lay-text="加入|不加入" checked>'
                    } else {
                        return '<input id="' + d.riskYearNow + '" partsName="' + d.partsName + '" functionCode="' + d.functionCode + '" supplierName="' + d.supplierName + '" type="checkbox" value="' + d.supplierCode + '"  lay-skin="switch" lay-filter="bjDemo" lay-text="加入|不加入">'
                    }
                }
            }, {
                title: '业务模块',
    ...
    form.on('submit(search1)', function(data) {
        if (data.field.date) {
            ...
            data.field.date = getPreMonth13(new Date(), 1);
        }
        saveArr2 = data.field;
    ...
    form.on('submit(supplierManage)', function(data) {
        var lastMonth = getPreMonth(new Date(data.field.riskTime.replace(/-/g, "/")), 0);
        if (data.field.riskTime == '' || data.field.riskTime == null) {
            layer.msg("请输入日期！");
            return;
        }
    ...
    function initEcharts(url, id, idPie, index, riskYear, functionName, supplierName, supplierCode) {
        admin.req("api/risk/monitior/" + url + "-status?riskYear=" + riskYear + "&functionNameList=" + functionName + "&supplierCode=" + supplierCode, {}, function(res) {
            if (res.ok) {
                var data = res.data;
                var prrCategory = [];
                var prrDecrease = [];
                ...
                var change =(preMonthData == 0 && currentMonthData==0)?0:preMonthData == 0 ? 100 : parseInt(((currentMonthData - preMonthData) / preMonthData) * 100);
                ...
                var optionpie1 = {
                    ...
                    graphic: {
                        ...
                        top: 'center',
                        style: {
                            text:(preMonthData == 0 && currentMonthData==0)?'-':change >= 0 ? '增幅' + change + '%' : '降幅' + -change + '%'
                        }
                    },
                    grid: {
                        bottom: -100
                ...
                curChart.on('click', function(params) {
                    ...
                    if (params.dataIndex == 11) {
                        ...
                        var clickChange = (preMonthData == 0 && currentMonthData==0)?0:preMonthData == 0 ? 100 : parseInt(((currentMonthData - preMonthData) / preMonthData) * 100);
                    } else {
                    ...
                    var optionpie = {
                        ...
                        graphic: {
                            ...
                            top: 'center',
                            style: {
                                text:(preMonthData == 0 && currentMonthData==0)?'-':  clickChange >= 0 ? '增幅' + clickChange + '%' : '降幅' + -clickChange + '%'
                            }
                        },
                        /*grid: {
                            bottom: 0
    ...
    function getMonitorBarOption(params) {
        console.log(params)
        var allData = params.prrDecrease.concat(params.prrKeep).concat(params.prrIncrease);
        var maxData = Math.max.apply(Math, allData);
    ...
    //风险供应商内部数据
    $("#exportInsideFile").click(function() {
        popMenu1();
    });
    //风险供应商内部数据弹出框
    function popMenu1() {
        admin.popupCenter({
            title: "风险供应商内部数据",
            area: '60%',
            path: "components/tpl/risk_monitor_inside.html",
            finish: function() {
    ...
    form.on('switch(bjDemo)', function(obj) {
        ...
        RiskSupplierMonitorAnalysisVO.partsName = obj.elem.attributes[1].value;
        RiskSupplierMonitorAnalysisVO.functionCode = obj.elem.attributes[2].value;
        RiskSupplierMonitorAnalysisVO.supplierName = obj.elem.attributes[3].value;
    ...
    function updateState(RiskSupplierMonitorAnalysisVO) {
        ...
        admin.req("api/risk/monitior/risk-supplier/update", RiskSupplierMonitorAnalysisVOList, function(res) {
            if (res.ok) {
    ...
    function supplierManage() {
        layer.open({
            ...
            success: function(layero) {
                ...
                var tableR = getSupplierManageCols(getPreMonth2(new Date(), 0), getPreMonth13(new Date(), 0));
    ...
    function exportExcel(file) {
        ...
        xhr.open('POST', admin.formatUrl("/api/risk/monitior/risk-supplier/excel"), true);
    ...
    // 执行内部风险预警
    laydate.render({
        elem: '#executeInsideTask',
        format: '执行内部风险预警',
        type: 'date',
        done: function(value, date, endDate) {
            var selectDate = date.year + (date.month < 10 ? '0' + date.month : date.month) + '01';
            admin.req("/api/risk/monitior/inside-risk/insidebyhand?riskTime=" + selectDate, {}, function(res) {
                if (!res.ok) {
                    return layer.msg(res.message);
                }
            });
            admin.removeLoading('body');
            layer.msg('内部风险预警任务正在执行中，请稍候再查阅数据');
        }
    });
